import { CalendarCheck, FileText, ListChecks, UsersThree } from "@phosphor-icons/react/dist/ssr";
...
export default async function PortalPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-stone-50">
      <header className="bg-[radial-gradient(100%_140%_at_50%_0%,#0b7a49_0%,#054f30_100%)] pb-14 pt-10 text-white">
        <div className="mx-auto max-w-2xl px-4">
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-brand-200">
            {tenantName} · client portal
          </p>
      ...
      <div className="mx-auto -mt-8 flex max-w-2xl flex-col gap-5 px-4 pb-10">
        <section className="rounded-xl border border-stone-200/70 bg-white p-5 shadow-[0_1px_2px_rgb(41_37_36/0.04),0_2px_8px_rgb(41_37_36/0.04)]">
          <dl className="grid grid-cols-2 gap-x-3 gap-y-4 text-sm">
            ...
            <div>
              <dt className="mb-1 text-xs uppercase tracking-wide text-stone-400">
                Purchase price
              </dt>
          ...
          <section className="rounded-xl border border-stone-200/70 bg-white p-5 shadow-[0_1px_2px_rgb(41_37_36/0.04),0_2px_8px_rgb(41_37_36/0.04)]">
            <div className="mb-3 flex items-baseline justify-between">
              ...
              <span className="text-xs tabular-nums text-stone-400">
                {txn.tasks.filter((t) => t.status === "DONE").length} of {txn.tasks.length} steps
                done
              </span>
            </div>
        ...
        <footer className="mt-2 text-center text-xs text-stone-400">
          Shared by {tenantName} · read-only view · powered by{" "}
          <a
