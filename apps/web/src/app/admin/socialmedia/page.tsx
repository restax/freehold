...
import { ASSET_GROUPS, PRESENTER_PROMPTS, SOCIAL_POSTS, type SocialPost } from "@/lib/social-kit";
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8 sm:px-6">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm leading-relaxed text-stone-600">
          {SOCIAL_POSTS.length} ready-to-post messages and{" "}
          {ASSET_GROUPS.reduce((n, g) => n + g.items.length, 0)} images. Copy a post, grab the image
          next to it, and go. Every claim here matches what the site says today.
        </p>
      </div>
      ...
      {/* Presenter photos: prompts for the one shot not made yet ----------- */}
      <section className={card}>
        <h2 className="flex items-center gap-2 font-medium">
          <Sparkle size={17} weight="fill" className="text-brand-700" aria-hidden />
          Making more presenter photos
        </h2>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-stone-600">
          The first three shots are in the Presenter photos gallery above. They were generated in
          Higgsfield, and where a shot shows a laptop screen, that screen is a real Freehold
          screenshot warped into perspective and composited in afterward, not generated — an image
          model asked to draw a specific interface invents one, with plausible chrome and garbled
          text. The trick to keeping the same woman across shots is to hold the character paragraph
          word-for-word identical and change only the scene sentence, feeding the first image back
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {PRESENTER_PROMPTS.shots.slice(3).map((s) => (
      ...
      <section className={card}>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-stone-600">
          Every screenshot and cover on this page was captured from the live product with
          Playwright, so they can be regenerated whenever the UI changes rather than going stale.
          The three real photos (cheese, then-vs-now) are the only hand-picked stock images left on
          the site; the rest of the covers and cards are those photos or the presenter shots,
          cropped to size. Ask Claude to re-run the social asset pipeline and everything here is
          rebuilt from the current UI.
        </p>
      </section>
    </div>
  );
