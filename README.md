...
- **Security & platform** — TOTP two-factor auth with backup codes, at-rest document encryption (envelope AES-256-GCM), six-digit email verification on Cloud signups, per-workspace marketing sites on subdomains with logo upload and buy-/sell-side intake forms, CSV import, a REST API with signed webhooks, client invoicing (payment-agnostic — a tracked document and follow-up task, with an optional per-tenant ERPNext connection that creates the Sales Invoice there and mirrors its status back), in-app support tickets (a "report an issue" box in the sidebar tags itself with the page you were on; reply threads on both the tenant's own tickets page and an operator queue), and integrations: Zapier, Follow Up Boss, Twenty CRM, ERPNext, per-tenant Documenso, and a Claude Skill.
