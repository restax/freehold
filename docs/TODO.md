...
- [ ] Transaction page round 2 (from the same spec): custom-field collections/groups + highlight colors, task snooze/priority/sub-notes + folders, attachment placeholders/folders/multi-select email, date formula calculator with business-day math + domino re-computation when anchor dates change (tasks currently do NOT shift when dates change), ICS invites per date, customizable statuses/roles, address auto-lookup, global search + notifications in a top bar, agent-portal analytics charts. Emails tab stays honest until IMAP/SMTP ships (spec's Gmail/Outlook OAuth conflicts with the no-OAuth decision)
